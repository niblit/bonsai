    ...
    view! {
      <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div class="bg-zinc-800 p-6 rounded-2xl border-2 border-zinc-600 shadow-2xl flex flex-col items-center gap-4">
          ...
          <div class="flex gap-4">
            {options
              ...
              .map(|promo| {
                let piece = Piece::new(
                  team,
                  Kind::from_valid_promotions(promo),
                );
                view! {
                  <button
          ...
          // Cancel Button
          <button
            class="px-6 py-2 bg-[#DA8B8B] hover:bg-[#C43B3B] text-[#0A1329] rounded-lg text-sm font-medium transition-colors border border-red-500/30"
